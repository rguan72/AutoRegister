...
your entire crew. You are the last suriving member and your last
...
code to get the bomb out. If you get the code wrong 10 times then
...
pod before the whole ship explodes. It seems liek hardly and Gothons
...
The thrown bomb explodes and kills you as it hits the ground.""")

armory_death = Room("death", "The security system kicks in and shoots you.")
corridor_shoot_death = Room("death", "The Gothon shoots you first.")
corridor_dodge_death = Room("death", "You are bad at dodging. You get shot.")
...
def load_room(name):
    ...
    Who gets to set name? (Anyone) Can that expose a variable? (Yes: app.secret_key)
    """
    return globals().get(name)

def name_room(room):
    """
    Same possible security problem. Can you trust room? (No, anyone can set it)
    What's a better solution than this globals lookup? Checking type first
    """
    if 'Room' in type(room).__name__: # prevent us from revealing app.secret_key
